<?php
$objentttOne = new db_article_master;
$strWhere = "category_id=15 and sub_category_id=16 and active='Y'";
$entttOne    = $objentttOne->selectAll($strWhere, 0, 6);
?>
<div class="article-listing-border p-2 mt-3">
  ...
  <div class="row">
    <?php
      if ($entttOne[0] > 0):
        $i = 1;
        while ($objentttOnes = mysql_fetch_object($entttOne[1])):
        $ahp          = $objentttOnes->article_homepage_headline;
        $aID          = $objentttOnes->article_id;          
        $apu          = $objentttOnes->article_page_url;
        $apu1         = str_replace($old_pattern1s, $new_pattern1s, $apu);
        $aImage       = $objentttOnes->article_image;          
        $aDate        = $objentttOnes->article_date;         
