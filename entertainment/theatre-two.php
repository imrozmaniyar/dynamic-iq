<?php
$objentttTwo = new db_article_master;
$strWhere = "category_id=15 and sub_category_id=16 and active='Y'";
$entttTwo    = $objentttTwo->selectAll($strWhere, 7, 3);
?>
<section class="mt-3 pt-1">
  <div class="container clearfix">
    <h2 class="article-news-listing-title">  تھیٹر۔</h2>
<?php
  if ($entttTwo[0] > 0):
    $i = 1;
    while ($objentttTwos = mysql_fetch_object($entttTwo[1])):
    $ahp          = $objentttTwos->article_homepage_headline;
    $ahps         = $objentttTwos->article_short_description;
    $aID          = $objentttTwos->article_id;          
    $apu          = $objentttTwos->article_page_url;
    $apu1         = str_replace($old_pattern1s, $new_pattern1s, $apu);
    $aImage       = $objentttTwos->article_image1;          
    $aDate        = $objentttTwos->article_date;         
    $time         = strtotime($aDate);
    $month        = date("M",$time);
    $year         = date("Y",$time);
    $day          = date("j",$time);
    $aDate1       = $objentttTwos->article_date1;         
    $da           = strtotime($aDate1);
    $month1       = date('F', $da);
    $year1        = date("Y",$da);
    $day1         = date("d",$da);              
    $aTime        = $objentttTwos->article_time;
   $i=$i+1;  
   ?>    
    <div class="row mt-3">
