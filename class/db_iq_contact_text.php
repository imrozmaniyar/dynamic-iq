...
class db_iq_contact_text{
    var $_iq_aboutus_id;
    var $_iq_aboutus_text;
    
    
    function db_iq_contact_text($id=NULL)
    {
        $this->_iq_aboutus_id          = NULL;
        $this->_iq_aboutus_text   = NULL;
        $id = preg_replace('#[^0-9]#i', '', $id);
        if ($id):
            $Query = sprintf("Select * from " . TABLE_IQ_CONTACT_TEXT. " where iq_aboutus_id = %d",$id);
            $objDB = new database;
            $objDB->db_connect();
    ...
    function save()
    {
        if ($this->_iq_aboutus_id):
            ...
            //update
            $Query = sprintf("UPDATE " . TABLE_IQ_CONTACT_TEXT . " SET" ." iq_aboutus_text ='".replace_spl_chr($this->_iq_aboutus_text)."'"."WHERE iq_aboutus_id = %d", $iq_aboutus_id);
	     else:
            //insert
            $Query = "insert into " . TABLE_IQ_CONTACT_TEXT .
            "(
                    iq_aboutus_text
              )" .
            "values
    ...
    function selectAll($strWhere = NULL, $cur = NULL, $max = NULL){
        ...
        $Query = "Select * from " . TABLE_IQ_CONTACT_TEXT . $strWhere . " order by iq_aboutus_id DESC";
		...
		if($cur || $max):
	        $Query ="Select * from ".TABLE_IQ_CONTACT_TEXT.$strWhere." order by iq_aboutus_id DESC LIMIT $cur,$max";
	        $result = $objDB->db_query($Query);
        endif;
...
/////////Delete function Begins//////////////////////////////////////
    function delete(){
        if ($this->_iq_aboutus_id):
            $iq_aboutus_id = preg_replace('#[^0-9]#i', '', $this->_iq_aboutus_id);
            $Query = sprintf("Delete from " . TABLE_IQ_CONTACT_TEXT . " where iq_aboutus_id =%d",$iq_aboutus_id);
            $objDB = new database;
            $objDB->db_connect();
            return $objDB->db_query($Query);
        endif;
