...
// Class
class db_users_videos{
    var $_id;
    var $_username;
    var $_name;
    var $_position;
// Class constructor
function db_users_videos($id = NULL){
    $this->_id        = NULL;
    $this->_username  = NULL;
    $this->_name      = NULL;
    $this->_position  = NULL;
    ...
    if ($id):
        $Query           = sprintf("Select * from " . TABLE_USERS_VIDEOS. " where id = %d",$id);
        $objDB           = new database;
        $objDB           ->db_connect();
    ...
    // Returns class name
    function GetClassName(){return 'db_users_videos';}
    function save(){
        if ($this->_id):
            $id       = preg_replace('#[^0-9]#i', '', $this->_id);
            //update
            $Query    = sprintf("UPDATE " . TABLE_USERS_VIDEOS . " SET " .
            ...
            //insert
            $Query = "insert into " . TABLE_USERS_VIDEOS .
            "(
                username,
                name,
                position
    ...
    function selectAll($strWhere = NULL, $cur = NULL, $max = NULL){
        ...
        $Query = "Select * from " . TABLE_USERS_VIDEOS . $strWhere . " order by position ASC";
		...
		if($cur || $max):
            $Query ="Select * from ".TABLE_USERS_VIDEOS.$strWhere." order by position ASC LIMIT $cur,$max";
	        $result = $objDB->db_query($Query);
        endif;
    ...
    function delete(){
        if ($this->_id):
            $id = preg_replace('#[^0-9]#i', '', $this->_id);
            $Query = sprintf("Delete from " . TABLE_USERS_VIDEOS . " where id =%d",$id);
            $objDB = new database;
            $objDB->db_connect();
            return $objDB->db_query($Query);
        endif;
