<section class="mt-3">
  <div class="container clearfix">
    <div class="news-section-bg p-3">
      <div class="col-md-12"><h1 class="lifestyle-section-title"><a href="<?php echo $domain?>lifestyle/youth" Alt="Youth" Title="Youth"> طبقۂ نوجواں </a></h1></div>
        <div class="news-section-inner-bg p-3">
          ...
          <?php
              if ($youth[0] > 0):
                $i = 1;
                while ($objyouths = mysql_fetch_object($youth[1])):
                $ahp          = $objyouths->article_homepage_headline;
                $aID          = $objyouths->article_id;     
                $hu           = str_replace($old_pattern1s, $new_pattern1s,$objyouths->article_page_url);                       
                $aImage       = $objyouths->article_image;          
                $aDate        = $objyouths->article_date;         
                $cid          = $objyouths->category_id;
          ...
          <?php
            if ($youthSingle[0] > 0):
                $i = 1;
                while ($objyouthSingles = mysql_fetch_object($youthSingle[1])):
                $ahp          = $objyouthSingles->article_homepage_headline;
                $aID          = $objyouthSingles->article_id;          
                $aImage       = $objyouthSingles->article_image;          
                $hu           = str_replace($old_pattern1s, $new_pattern1s,$objyouthSingles->article_page_url);                        
                $aDate        = $objyouthSingles->article_date;         
                $ahps         = $objyouthSingles->article_short_description;
                $cid          = $objyouthSingles->category_id;
                $time         = strtotime($aDate);
                $month        = date("M",$time);
                $year         = date("Y",$time);  
          ...
          ?>    
              <div class="col-md-5 order-0 order-md-1">
                ...
                <a href="<?php echo $domain?>lifestyle/youth" class="text-right read-more mt-1" Alt="More" Title="More">مزید </a>
              </div>
          <?php
            endwhile;
          endif;  
