<section class="mt-3">
  <div class="container clearfix">
    <div class="news-section-bg p-3">
      <div class="col-md-12"><h1 class="lifestyle-section-title"><a href="<?php echo $domain?>lifestyle/tech" Alt="Tech" Title="Tech"> ٹیکنالوجی  </a></h1></div>
        <div class="news-section-inner-bg p-3">
          ...
          <?php
              if ($Tech[0] > 0):
                $i = 1;
                while ($objTechs = mysql_fetch_object($Tech[1])):
                $ahp          = $objTechs->article_homepage_headline;
                $aID          = $objTechs->article_id;     
                $hu           = str_replace($old_pattern1s, $new_pattern1s,$objTechs->article_page_url);                       
                $aImage       = $objTechs->article_image;          
                $aDate        = $objTechs->article_date;         
                $cid          = $objTechs->category_id;
          ...
          <?php
            if ($TechSingle[0] > 0):
                $i = 1;
                while ($objTechSingles = mysql_fetch_object($TechSingle[1])):
                $ahp          = $objTechSingles->article_homepage_headline;
                $aID          = $objTechSingles->article_id;          
                $aImage       = $objTechSingles->article_image;          
                $hu           = str_replace($old_pattern1s, $new_pattern1s,$objTechSingles->article_page_url);                        
                $aDate        = $objTechSingles->article_date;         
                $ahps         = $objTechSingles->article_short_description;
                $cid          = $objTechSingles->category_id;
                $time         = strtotime($aDate);
                $month        = date("M",$time);
                $year         = date("Y",$time);  
          ...
          ?>    
              <div class="col-md-5 order-0 order-md-1">
                ...
                <a href="<?php echo $domain?>lifestyle/tech" class="text-right read-more mt-1" alt="More" title="More">مزید </a>
              </div>
          <?php
            endwhile;
          endif;  
