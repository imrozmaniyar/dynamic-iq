<section class="mt-3">
  <div class="container clearfix">
    <div class="news-section-bg p-3">
      <div class="col-md-12"><h1 class="lifestyle-section-title"><a href="<?php echo $domain?>lifestyle/women" Alt="Women" Title="Women"> خواتین </a></h1></div>
        <div class="news-section-inner-bg p-3">
          ...
          <?php
              if ($women[0] > 0):
                $i = 1;
                while ($objwomens = mysql_fetch_object($women[1])):
                $ahp          = $objwomens->article_homepage_headline;
                $aID          = $objwomens->article_id;     
                $hu           = str_replace($old_pattern1s, $new_pattern1s,$objwomens->article_page_url);                       
                $aImage       = $objwomens->article_image;          
                $aDate        = $objwomens->article_date;         
                $cid          = $objwomens->category_id;
          ...
          <?php
            if ($womenSingle[0] > 0):
                $i = 1;
                while ($objwomenSingles = mysql_fetch_object($womenSingle[1])):
                $ahp          = $objwomenSingles->article_homepage_headline;
                $aID          = $objwomenSingles->article_id;          
                $aImage       = $objwomenSingles->article_image;          
                $hu           = str_replace($old_pattern1s, $new_pattern1s,$objwomenSingles->article_page_url);                        
                $aDate        = $objwomenSingles->article_date;         
                $ahps         = $objwomenSingles->article_short_description;
                $cid          = $objwomenSingles->category_id;
                $time         = strtotime($aDate);
                $month        = date("M",$time);
                $year         = date("Y",$time);  
 ...
 ?>
                ...
                <a href="<?php echo $domain?>lifestyle/women" class="text-right read-more mt-1" alt="More" Title="More">مزید </a>
                 <?php endif;?>
              </div>
          <?php
            endwhile;
