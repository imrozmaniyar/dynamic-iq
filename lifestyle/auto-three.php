<?php
$objenttThree = new db_article_master;
$strWhere = "category_id=18 and sub_category_id=24 and active='Y'";
$enttThree    = $objenttThree->selectAll($strWhere, 10, 3);
?>
<section class="mt-3">
  <div class="container clearfix">
    <h2 class="article-news-listing-title"> خواتین۔  </h2>
<?php
  if ($enttThree[0] > 0):
    $i = 1;
    while ($objenttThrees = mysql_fetch_object($enttThree[1])):
    $ahp          = $objenttThrees->article_homepage_headline;
    $ahps         = $objenttThrees->article_short_description;
    $aID          = $objenttThrees->article_id;          
    $apu          = $objenttThrees->article_page_url;
    $apu1         = str_replace($old_pattern1s, $new_pattern1s, $apu);
    $aImage       = $objenttThrees->article_image1;          
    $aDate        = $objenttThrees->article_date;         
    $time         = strtotime($aDate);
    $month        = date("M",$time);
    $year         = date("Y",$time);
    $day          = date("j",$time);   
    $aDate1       = $objenttThrees->article_date1;         
    $da           = strtotime($aDate1);
    $month1       = date('F', $da);
    $year1        = date("Y",$da);
    $day1         = date("d",$da);             
    $aTime        = $objenttThrees->article_time;
   $i=$i+1;  
 ?>    
    <div class="row mt-3">
      ...
      <a href="<?php echo $domain?>lifestyle/articles/<?php echo htmlspecialchars($apu1,ENT_QUOTES, 'UTF-8')?>-<?php echo htmlspecialchars($aID,ENT_QUOTES, 'UTF-8')?>"><div class="bg-grey-mobile"><img src="<?php echo htmlspecialchars($aImage,ENT_QUOTES, 'UTF-8')?>" class="media-object img-fluid d-block mx-auto align-self-center"></div></a>
      </div>
    </div>
<?php
  endwhile;
