<section>
  <div class="container clearfix">
    ...
    <?php
      if ($homep11[0] > 0):
        $i = 1;
          while ($objhomep11s = mysql_fetch_object($homep11[1])):
          $username           = $objhomep11s->username;
          $nameID             = $objhomep11s->name;
          if($username     =='Article'):
    ...
    ?>  
      <div class="row">
        ...
        <div class="col-md-6 order-1 order-md-2">
          <a href="<?php echo htmlspecialchars($url,ENT_QUOTES, 'UTF-8')?>" class="home-href">
            <img src="<?php echo htmlspecialchars($hi,ENT_QUOTES, 'UTF-8')?>" class="img-fluid d-block mx-auto first-section-img" alt=""></a><div class="latest-text">Latest</div></div>
      </div>
      <?php
        endwhile;
      endif;  
      ...
      if ($homep112[0] > 0):
        $i = 1;
          while ($objhomep112s = mysql_fetch_object($homep112[1])):
          $username           = $objhomep112s->username;
          $nameID             = $objhomep112s->name;
          if($username     =='Article'):
            $objAmaster    = new db_article_master($nameID);
            $hid           = $objAmaster->Get_article_id();
            $ht            = $objAmaster->Get_article_homepage_headline();
            $hd            = $objAmaster->Get_article_short_description();
            $hu            = str_replace($old_pattern1s, $new_pattern1s,$objAmaster->Get_article_page_url());
            $hi            = $objAmaster->Get_article_image1();
            $hdate         = $objAmaster->Get_article_date();
            $time          = strtotime($hdate);
            $month         = date("M",$time);
            $year          = date("Y",$time);
      ...
      ?>        
        <div class="col-md-3 zoom col-6 mt-3 mt-md-3">
          <a href="<?php echo htmlspecialchars($url,ENT_QUOTES, 'UTF-8')?>" class="home-href">
            <img src="<?php echo htmlspecialchars($hi,ENT_QUOTES, 'UTF-8')?>" class="img-fluid mx-auto d-block first-section-sub-section-img" alt="">
