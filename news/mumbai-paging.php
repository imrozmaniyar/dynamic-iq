...
$last_blog_id = $_POST['last_blog_id'];
$objMumbaiPaging  = new db_article_master;
$strWhere1 = "article_id < $last_blog_id and category_id=14 and sub_category_id=30 and active='Y'";
$MumbaiPaging     = $objMumbaiPaging->selectAll($strWhere1, 0, 3);
?>

<?php
  if ($MumbaiPaging[0] > 0):
    $i = 1;
    while ($objMumbaiPagings = mysql_fetch_object($MumbaiPaging[1])):
    $ahp          = $objMumbaiPagings->article_homepage_headline;
    $ahps         = $objMumbaiPagings->article_short_description;
    $aID          = $objMumbaiPagings->article_id;          
    $apu          = $objMumbaiPagings->article_page_url;
    $apu1         = str_replace($old_pattern1s, $new_pattern1s, $apu);
    $aImage       = $objMumbaiPagings->article_image1;          
    $aDate        = $objMumbaiPagings->article_date;         
    $time         = strtotime($aDate);
    $month        = date("M",$time);
    $year         = date("Y",$time);
    $day          = date("j",$time);          
    $aDate1       = $objMumbaiPagings->article_date1;         
    $da           = strtotime($aDate1);
    $month1       = date('F', $da);
    $year1        = date("Y",$da);
    $day1         = date("d",$da);
    $aTime        = $objMumbaiPagings->article_time;
...
?>
 ...
 <?php if ($MumbaiPaging[0] > 3) { ?><div class="mt-5 mb-5"><div id="remove_row"><img src="<?php echo $domain?>images/load-more-btn.png" class="img-fluid d-block mx-auto align-self-center" data-bid="<?php echo $aID; ?>" id="btn_more"></div>
 </div>
<?php }else{ ?>
