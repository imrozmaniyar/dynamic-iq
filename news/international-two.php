<?php
$objnewsnationalTwoInter = new db_article_master;
$strWhere = "category_id=14 and sub_category_id=12 and article_date1 <='$shedate' and article_epoch<=$timestamp and active='Y'";
$newsnationalTwoInter    = $objnewsnationalTwoInter->selectAll($strWhere, 7, 3);
?>
<section class="mt-3 pt-1">
  <div class="container clearfix">
    <h2 class="article-news-listing-title" alt="International" title="International"> عالم  </h2>
<?php
  if ($newsnationalTwoInter[0] > 0):
    $i = 1;
    while ($objnewsnationalTwoInters = mysql_fetch_object($newsnationalTwoInter[1])):
    $ahp          = $objnewsnationalTwoInters->article_homepage_headline;
    $ahps         = $objnewsnationalTwoInters->article_short_description;
    $aID          = $objnewsnationalTwoInters->article_id;          
    $apu          = $objnewsnationalTwoInters->article_page_url;
    $apu1         = str_replace($old_pattern1s, $new_pattern1s, $apu);
    $aImage       = $objnewsnationalTwoInters->article_image1;          
    $aDate        = $objnewsnationalTwoInters->article_date;         
    $time         = strtotime($aDate);
    $month        = date("M",$time);
    $year         = date("Y",$time);
    $day          = date("j",$time); 
    $aDate1       = $objnewsnationalTwoInters->article_date1;         
    $da           = strtotime($aDate1);
    $month1       = date('F', $da);
    $year1        = date("Y",$da);
    $day1         = date("d",$da);             
    $aTime        = $objnewsnationalTwoInters->article_time;
   $i=$i+1;  
   ?>    
    <div class="row mt-3">
