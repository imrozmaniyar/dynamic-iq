<?php include('../top.php'); ?>
      ...
      <div class="container clearfix">
        <nav aria-label="breadcrumb" class="clearfix">
          <ol class="breadcrumb float-right mb-0 pb-0 news-breadcrumb">
            <li class="breadcrumb-item font-weight-bold text-black" Alt="National" title="National"> قومی  </a></li>
        ...
        <?php include('national-one.php'); ?>        
      </div>  
    </section>
    <!-- first section -->
    <!-- article-news-listing Section -->
    <!-- article-news-listing Section -->
    <?php include('national-two.php'); ?>
    ...
    <!-- photo section -->
    <!-- article-news-listing Section -->
    <?php include('national-three.php'); ?>
    <?php $isMobile = (bool) strpos($_SERVER['HTTP_USER_AGENT'],'Mobile'); if ($isMobile):?>
...
<?php
$objnewsnationalFour = new db_article_master;
$strWhere = "category_id=14 and sub_category_id=11 and article_date1 <='$shedate' and article_epoch<=$timestamp and active='Y'";
$newsnationalFour    = $objnewsnationalFour->selectAll($strWhere, 13, 3);
?>    
<section class="mt-3">
  <div class="container clearfix" id="load_data_table">
    <h2 class="article-news-listing-title" alt="National" title="National"> وطن</h2>
<?php
  if ($newsnationalFour[0] > 0):
    $i = 1;
    while ($objnewsnationalFours = mysql_fetch_object($newsnationalFour[1])):
    $ahp          = $objnewsnationalFours->article_homepage_headline;
    $ahps         = $objnewsnationalFours->article_short_description;
    $aID          = $objnewsnationalFours->article_id;          
    $apu          = $objnewsnationalFours->article_page_url;
    $apu1         = str_replace($old_pattern1s, $new_pattern1s, $apu);
    $aImage       = $objnewsnationalFours->article_image1;          
    $aDate        = $objnewsnationalFours->article_date;         
    $time         = strtotime($aDate);
    $month        = date("M",$time);
    $year         = date("Y",$time);
    $day          = date("j",$time); 
    $aDate1       = $objnewsnationalFours->article_date1;         
    $da           = strtotime($aDate1);
    $month1       = date('F', $da);
    $year1        = date("Y",$da);
    $day1         = date("d",$da);
...
?>      
      ...
      <?php if ($newsnationalFour[0] != 3) { ?><div id="remove_row"><div class="mt-5 mb-5"><img src="<?php echo $domain?>images/load-more-btn.png" class="img-fluid d-block mx-auto align-self-center" data-bid="<?php echo $aID; ?>" id="btn_more"></div></div><?php } ?>   
</section>
