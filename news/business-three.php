<?php
$objnewsBusinessThree = new db_article_master;
$strWhere = "category_id=14 and sub_category_id=13 and active='Y'";
$newsBusinessThree    = $objnewsBusinessThree->selectAll($strWhere, 10, 3);
?>
<section class="mt-3">
  <div class="container clearfix">
    <h2 class="article-news-listing-title"> کاروبار۔  </h2>
<?php
  if ($newsBusinessThree[0] > 0):
    $i = 1;
    while ($objnewsBusinessThrees = mysql_fetch_object($newsBusinessThree[1])):
    $ahp          = $objnewsBusinessThrees->article_homepage_headline;
    $ahps         = $objnewsBusinessThrees->article_short_description;
    $aID          = $objnewsBusinessThrees->article_id;          
    $apu          = $objnewsBusinessThrees->article_page_url;
    $apu1         = str_replace($old_pattern1s, $new_pattern1s, $apu);
    $aImage       = $objnewsBusinessThrees->article_image1;          
    $aDate        = $objnewsBusinessThrees->article_date;         
    $time         = strtotime($aDate);
    $month        = date("M",$time);
    $year         = date("Y",$time);
    $day          = date("j",$time); 
    $aDate1       = $objnewsBusinessThrees->article_date1;         
    $da           = strtotime($aDate1);
    $month1       = date('F', $da);
    $year1        = date("Y",$da);
    $day1         = date("d",$da);             
    $aTime        = $objnewsBusinessThrees->article_time;
   $i=$i+1;  
 ?>    
    <div class="row mt-3">
