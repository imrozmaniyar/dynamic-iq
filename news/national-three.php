...
<?php
  if ($newsnationalThree[0] > 0):
    $i = 1;
    while ($objnewsnationalThrees = mysql_fetch_object($newsnationalThree[1])):
    $ahp          = $objnewsnationalThrees->article_homepage_headline;
    $ahps         = $objnewsnationalThrees->article_short_description;
    $aID          = $objnewsnationalThrees->article_id;          
    $apu          = $objnewsnationalThrees->article_page_url;
    $apu1         = str_replace($old_pattern1s, $new_pattern1s, $apu);
    $aImage       = $objnewsnationalThrees->article_image1;          
    $aDate        = $objnewsnationalThrees->article_date;         
    $time         = strtotime($aDate);
    $month        = date("M",$time);
    $year         = date("Y",$time);
    $day          = date("j",$time);     
    $aDate1       = $objnewsnationalThrees->article_date1;         
    $da           = strtotime($aDate1);
    $month1       = date('F', $da);
    $year1        = date("Y",$da);
    $day1         = date("d",$da);
