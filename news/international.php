<?php include('../top.php'); ?>
      ...
      <div class="container clearfix">
        <nav aria-label="breadcrumb" class="clearfix">
          <ol class="breadcrumb float-right mb-0 pb-0 news-breadcrumb">
            <li class="breadcrumb-item font-weight-bold text-black" alt="International" title="International"> عالم </a></li>
        ...
        <?php include('international-one.php'); ?>        
      </div>  
    </section>
    <!-- first section -->
    <!-- article-news-listing Section -->
    <!-- article-news-listing Section -->
    <?php include('international-two.php'); ?>
   ...
   <!--  <div class="horizontal-border mt-3"></div> -->
    ...
    <!-- photo section -->
    <!-- article-news-listing Section -->
    <?php include('international-three.php'); ?>
    <?php $isMobile = (bool) strpos($_SERVER['HTTP_USER_AGENT'],'Mobile'); if ($isMobile):?>
...
<?php
$objnewsnationalFourInter = new db_article_master;
$strWhere = "category_id=14 and sub_category_id=12 and article_date1 <='$shedate' and article_epoch<=$timestamp and active='Y'";
$newsnationalFourInter    = $objnewsnationalFourInter->selectAll($strWhere, 13, 3);
?>    
<section class="mt-3">
...
<?php
  if ($newsnationalFourInter[0] > 0):
    $i = 1;
    while ($objnewsnationalFourInters = mysql_fetch_object($newsnationalFourInter[1])):
    $ahp          = $objnewsnationalFourInters->article_homepage_headline;
    $ahps         = $objnewsnationalFourInters->article_short_description;
    $aID          = $objnewsnationalFourInters->article_id;          
    $apu          = $objnewsnationalFourInters->article_page_url;
    $apu1         = str_replace($old_pattern1s, $new_pattern1s, $apu);
    $aImage       = $objnewsnationalFourInters->article_image1;          
    $aDate        = $objnewsnationalFourInters->article_date;         
    $time         = strtotime($aDate);
    $month        = date("M",$time);
    $year         = date("Y",$time);
    $day          = date("j",$time);    
    $aDate1       = $objnewsnationalFourInters->article_date1;         
    $da           = strtotime($aDate1);
    $month1       = date('F', $da);
    $year1        = date("Y",$da);
    $day1         = date("d",$da);
    $aTime        = $objnewsnationalFourInters->article_time;
   $i=$i+1;  
 ?>    
      <div class="row mt-3">
