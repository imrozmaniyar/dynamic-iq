      ...
      <div class="container clearfix">
        <nav aria-label="breadcrumb" class="clearfix">
          <ol class="breadcrumb float-right mb-0 pb-0 news-breadcrumb">
            <li class="breadcrumb-item font-weight-bold"><a href="#"> ممبئی </a></li>
...
<?php
$objnewsMumbaiFour = new db_article_master;
$strWhere = "category_id=14 and sub_category_id=30 and active='Y'";
$newsMumbaiFour    = $objnewsMumbaiFour->selectAll($strWhere, 13, 3);
?>    
<section class="mt-3">
  <div class="container clearfix" id="load_data_table">
    <h2 class="article-news-listing-title"> قومی  </h2>
<?php
  if ($newsMumbaiFour[0] > 0):
    $i = 1;
    while ($objnewsMumbaiFours = mysql_fetch_object($newsMumbaiFour[1])):
    $ahp          = $objnewsMumbaiFours->article_homepage_headline;
    $ahps         = $objnewsMumbaiFours->article_short_description;
    $aID          = $objnewsMumbaiFours->article_id;          
    $apu          = $objnewsMumbaiFours->article_page_url;
    $apu1         = str_replace($old_pattern1s, $new_pattern1s, $apu);
    $aImage       = $objnewsMumbaiFours->article_image1;          
    $aDate        = $objnewsMumbaiFours->article_date;         
    $time         = strtotime($aDate);
    $month        = date("M",$time);
    $year         = date("Y",$time);
    $day          = date("j",$time);          
    $aDate1       = $objnewsMumbaiFours->article_date1;         
    $da           = strtotime($aDate1);
    $month1       = date('F', $da);
    $year1        = date("Y",$da);
    $day1         = date("d",$da);
    $aTime        = $objnewsMumbaiFours->article_time;
   $i=$i+1;  
 ?>    
      <div class="row mt-3">
