...
$objcat = new db_sub_category_master();
switch($mode):
    case "New":
	//$checkExist = $objcat->selectAll("catgory_master_name='" . $cname . "");
	$checkExist = $objcat->selectAll("sub_category_master_name ='" . $cname ."'");
	if ($checkExist[0] > 0):
		header("Location:sub_category-list.php?msgid=124");
	    exit();
	else:
        $objcat->set_category_master_id($cid);
        $objcat->set_sub_category_master_name_urdu($uname);
        $objcat->set_sub_category_master_name($cname);
        $objcat->set_sub_category_master_meta_title($ctitle);
        $objcat->set_sub_category_master_meta_desc($cdesc);
        $objcat->set_sub_category_master_meta_keywords($ckeys);
        $objcat->set_sub_category_master_date($cdate);
        $objcat->set_sub_category_master_date1($cdate);
    ...
    case "Edit":
            ...
            $objcat->set_sub_category_master_meta_desc($cdesc);
            $objcat->set_sub_category_master_meta_keywords($ckeys);
            $objcat->set_sub_category_master_date($olddate);
            $objcat->set_sub_category_master_date1($mdate);
    ...
    case "Delete":
      $cid       = intval($_GET['cid']);
        ...
        $Ctrl = "chk$i";
            if($_POST[$Ctrl]):
                $objcat->set_sub_category_master_id($_POST[$Ctrl]);
                    if($objcat->delete()):
                        $msgid = 101;
                    endif;
            endif;
    ...
    break;
endswitch;
header("Location:subcategory-list.php?cid=$cid&msgid=$msgid");
exit();
