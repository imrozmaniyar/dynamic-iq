...
$poetry_name          = mysql_real_escape_string($_POST['txttitle'], $link);
$poetry_desc   = mysql_real_escape_string($_POST['txtstory'], $link);
$mdate             = date('Y-m-d');
...
$poetry_master_image1 = mysql_real_escape_string($_POST['txtPreviousImage1'], $link);
$poetry_master_image2 = mysql_real_escape_string($_POST['txtPreviousImage2'], $link);
$chkactive = mysql_real_escape_string($_POST['cbostatus'], $link);
$objpk = new db_poetry_master();
$msgid = 104;
...
if(($image1_size > 2000) || (($width > 675) || ($height > 450) )):
    $urlredirect = "poetry-list.php?msgid=$msgid";
...
switch($mode):
    case "New":
    if($_FILES['txtimage']['name'] == ""):
        $poetry_master_image         = "noimage.jpg";
    else:
        ...
        $poetry_master_image          = $newFileName .'_d'. '.' .$extension;
        $poetry_master_imagee1         = $newFileName .'_l'. '.' .$extension;
        $poetry_master_image2         = $newFileName .'_s'. '.' .$extension;

        $new_file_name = $poetry_master_image;
        ...
        $new_file_name1 = $poetry_master_image1;
        ...
        $new_file_name2 = $poetry_master_image2;
        ...
        ///for 180 x 120 Size
            ...
            $objpk->Set_poetry_master_title($poetry_name);
            $objpk->Set_poetry_master_image($pathname.$poetry_master_image);
            $objpk->Set_poetry_master_image1($pathname1.$poetry_master_image1);
            $objpk->Set_poetry_master_image2($pathname2.$poetry_master_image2);
			$objpk->Set_poetry_master_desc($poetry_desc);
            $objpk->Set_poetry_master_date($mdate);
    ...
    case "Edit":
        if ($_FILES['txtimage']['name'] == ""):
                    if ($txtPreviousImage != ""):
                        $poetry_master_image = $txtPreviousImage;
                    else:
                        $poetry_master_image = "noimage.jpg";
                    endif;                  
                else:
                    ...
                    $newFileName        = substr($filename1, 0 , (strrpos($filename1, ".")));                    
                    $poetry_master_image        = $newFileName .'_d'. '.' .$extension;
                    $poetry_master_image1       = $newFileName .'_l'. '.' .$extension;
                    $poetry_master_image2       = $newFileName .'_s'. '.' .$extension;
                    $new_file_name = $poetry_master_image;
                    $pathname = '../images/' . date("Y") . '/' . date("M") . '/';
                    if (!is_dir($pathname)) {
            ...
            $objpk->Set_poetry_master_id($tid);
            $objpk->Set_poetry_master_title($poetry_name);
            $objpk->Set_poetry_master_image($pathname.$poetry_master_image);
            $objpk->Set_poetry_master_image1($pathname1.$poetry_master_image1);
            $objpk->Set_poetry_master_image2($pathname2.$poetry_master_image2);
	...
	for($i=1;$i<=$Ctrls;$i++){
            $Ctrl = "chk$i";
             if($_POST[$Ctrl]):
                $objpk->Set_poetry_master_id($_POST[$Ctrl]);
		$strWhere ="poetry_master_id =".$_POST[$Ctrl];
		$objgalleries = $objpk->selectAll($strWhere,$cur,$max);
                if($objpk->delete()):
			$delFile = mysql_fetch_object($objgalleries[1]);
			$file_delete=$destination.$delFile->poetry_master_image;
			foreach(glob($destination.'*') as $image)
			    {
				if(($delFile->poetry_master_image!== "noimage.jpg")):
				    if($file_delete == $image):
					@unlink(realpath($destination.$delFile->poetry_master_image));
				    endif;
				endif;
			    }
            $file_delete=$destination.$delFile->poetry_master_image1;
            foreach(glob($destination.'*') as $image)
                {
                if(($delFile->poetry_master_image1!== "noimage.jpg")):
                    if($file_delete == $image):
                    @unlink(realpath($destination.$delFile->poetry_master_image1));
                    endif;
                endif;
                }  
            $file_delete=$destination.$delFile->poetry_master_image2;
            foreach(glob($destination.'*') as $image)
                {
                if(($delFile->poetry_master_image2!== "noimage.jpg")):
                    if($file_delete == $image):
                    @unlink(realpath($destination.$delFile->poetry_master_image2));
                    endif;
                endif;
                }                               
			$msgid = 101;
